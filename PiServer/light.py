...
class Light:  
    ...
    def setState(self, state = False):
        ...
        #print("Light " + self.name + " is now " + self.state and "ON" or "OFF")
        print("Light " + self.name + " is now " + str(self.state))
    ...
    def onebyone_blink(self, times = 1, doBackwards = False): # One By One Blink, support for backwards
        ...
        for i in range(times):
            if doBackwards == True:
                ...
                for i in range(len(self.Lights)-1, 0, -1):     # Goes down the list of lights and toggles them again
                    self.getLight(i).toggle()
                    self.sleepFactor()
